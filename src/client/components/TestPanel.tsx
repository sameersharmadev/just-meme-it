...
export const TestPanel = () => {
  ...
  const testVoteStatus = () => {
    ...
    void runTest('Vote Status', () => fetch(`/api/test/vote-status/${selectedOderId}`));
  };

  const testUserStatus = () => void runTest('User Status', () => fetch('/api/test/user-status'));

  const testUpdateStreak = () =>
    void runTest('Update Streak', () =>
  ...
  const testRecordWin = () =>
    void runTest('Record Win (+100 pts)', () =>
  ...
  const testAddLifetimeScore = () =>
    void runTest('Add Lifetime Score (+10)', () =>
  ...
  const testDailyLeaderboard = () =>
    void runTest('Daily Leaderboard', () => fetch('/api/test/leaderboard/daily?limit=10'));

  const testLifetimeLeaderboard = () =>
    void runTest('Lifetime Leaderboard', () => fetch('/api/test/leaderboard/lifetime?limit=10'));

  return (
    <div className="fixed top-0 right-0 w-80 h-full bg-white border-l border-gray-200 p-4 overflow-y-auto shadow-lg z-[60]">
      ...
      <div className="space-y-2 mb-4">
        ...
        <button
          onClick={() => void testGetSubmissions()}
          disabled={loading}
